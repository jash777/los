...
async function testLoanFunding() {
    console.log('Testing loan funding service...');
    ...
    try {
        ...
        // Check if we can manually trigger loan funding
        console.log('Checking if we can manually trigger loan funding...');
        try {
            const loanFundingResponse = await axios.post(`http://localhost:3000/api/loan-funding/process/${applicationNumber}`);
            console.log('Loan funding response:', loanFundingResponse.data);
        } catch (error) {
            console.log('Loan funding error:', error.response?.data || error.message);
        }
